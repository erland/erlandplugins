...
package Plugins::PortableSync::ConfigManager::TemplateContentParser;

use strict;
use base qw(Slim::Utils::Accessor);
use Plugins::PortableSync::ConfigManager::ContentParser;
our @ISA = qw(Plugins::PortableSync::ConfigManager::ContentParser);
	...
	if(defined($template->{lc($self->pluginId).'_plugin_template'})) {
		...
		my @pluginDirs = Slim::Utils::OSDetect::dirsFor('Plugins');
                for my $plugindir (@pluginDirs) {
                        if( -d catdir($plugindir,"PortableSync","Templates") && -e catfile($plugindir,"PortableSync","Templates",$templateFile)) {
                                if(defined($path)) {
                                        my $prevTimestamp = (stat ($path) )[9];
                                        my $thisTimestamp = (stat (catfile($plugindir,"PortableSync","Templates",$templateFile)) )[9];
                                        if($prevTimestamp<=$thisTimestamp) {
                                                $path = catfile($plugindir,"PortableSync","Templates",$templateFile);
                                        }
                                }else {
					$path = catfile($plugindir,"PortableSync","Templates",$templateFile);
				}
