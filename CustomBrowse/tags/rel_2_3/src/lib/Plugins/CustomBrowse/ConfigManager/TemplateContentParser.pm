...
package Plugins::CustomBrowse::ConfigManager::TemplateContentParser;

use strict;
use base 'Class::Data::Accessor';
use Plugins::CustomBrowse::ConfigManager::ContentParser;
our @ISA = qw(Plugins::CustomBrowse::ConfigManager::ContentParser);
...
sub loadTemplate {
	...
	if(defined($template->{lc($self->pluginId).'_plugin_template'})) {
		...
		for my $plugindir (@pluginDirs) {
			if( -d catdir($plugindir,"CustomBrowse","Templates") && -e catfile($plugindir,"CustomBrowse","Templates",$templateFile)) {
				if(defined($path)) {
					my $prevTimestamp = (stat ($path) )[9];
					my $thisTimestamp = (stat (catfile($plugindir,"CustomBrowse","Templates",$templateFile)) )[9];
					if($prevTimestamp<=$thisTimestamp) {
						$path = catfile($plugindir,"CustomBrowse","Templates",$templateFile);
					}
				}else {
					$path = catfile($plugindir,"CustomBrowse","Templates",$templateFile);
				}
