...
sub getAllTracksWeb {
	...
	if(defined($params->{'artist'})) {
	    ...
	    $sql = "select tracks.id,track_statistics.playCount,track_statistics.added,track_statistics.lastPlayed,track_statistics.rating from tracks join contributor_track on tracks.id=contributor_track.track and contributor_track.contributor=$artist left join track_statistics on tracks.url = track_statistics.url where tracks.audio=1 group by tracks.url order by tracks.album asc,tracks.tracknum asc limit $listLength;";
	    $params->{'statisticparameters'} = "&artist=$artist";
	}elsif(defined($params->{'album'})) {
		my $album = $params->{'album'};
	    $sql = "select tracks.id,track_statistics.playCount,track_statistics.added,track_statistics.lastPlayed,track_statistics.rating from tracks left join track_statistics on tracks.url = track_statistics.url where tracks.audio=1 and tracks.album=$album order by tracks.tracknum";
	    $params->{'statisticparameters'} = "&album=$album";
	}elsif(defined($params->{'genre'})) {
		my $genre = $params->{'genre'};
	    $sql = "select tracks.id,track_statistics.playCount,track_statistics.added,track_statistics.lastPlayed,track_statistics.rating from tracks join genre_track on tracks.id=genre_track.track and genre_track.genre=$genre left join track_statistics on tracks.url = track_statistics.url where tracks.audio=1 order by tracks.album asc,tracks.tracknum asc limit $listLength;";
	    $params->{'statisticparameters'} = "&genre=$genre";
	}elsif(defined($params->{'year'})) {
		my $year = $params->{'year'};
	    $sql = "select tracks.id,track_statistics.playCount,track_statistics.added,track_statistics.lastPlayed,track_statistics.rating from tracks left join track_statistics on tracks.url = track_statistics.url where tracks.audio=1 and tracks.year=$year order by tracks.album asc,tracks.tracknum asc limit $listLength;";
	    ...
	    $sql = "select tracks.id,track_statistics.playCount,track_statistics.added,track_statistics.lastPlayed,track_statistics.rating from tracks left join track_statistics on tracks.url = track_statistics.url where tracks.audio=1 order by tracks.album asc,tracks.tracknum asc limit $listLength;";
	}
...
sub getAllTracks {
	...
	if(Slim::Utils::Prefs::get("plugin_trackstat_dynamicplaylist_norepeat")) {
		$sql = "select tracks.id from tracks left join track_statistics on tracks.url = track_statistics.url left join dynamicplaylist_history on tracks.id=dynamicplaylist_history.id where tracks.audio=1 and dynamicplaylist_history.id is null order by tracks.album asc,tracks.tracknum asc limit $listLength;";
	}else {
		$sql = "select tracks.id from tracks left join track_statistics on tracks.url = track_statistics.url where tracks.audio=1 order by tracks.album asc, tracks.tracknum asc limit $listLength;";
	}
...
{
	return "
PLUGIN_TRACKSTAT_SONGLIST_ALL
	EN	All songs

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORARTIST_SHORT
	EN	Songs

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORARTIST
	EN	All songs by: 

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORALBUM_SHORT
	EN	Songs

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORALBUM
	EN	All songs from: 

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORGENRE_SHORT
	EN	Songs

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORGENRE
	EN	All songs in: 

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORYEAR_SHORT
	EN	Songs

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORYEAR
	EN	All songs from: 

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORPLAYLIST_SHORT
	EN	Songs

PLUGIN_TRACKSTAT_SONGLIST_ALL_FORPLAYLIST
	EN	All songs in: 

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS
	EN	All albums

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORARTIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORARTIST
	EN	All albums by: 

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORGENRE_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORGENRE
	EN	All albums in: 

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORYEAR_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORYEAR
	EN	All albums from: 

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORPLAYLIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_ALLALBUMS_FORPLAYLIST
	EN	All albums in: 

PLUGIN_TRACKSTAT_SONGLIST_ALLARTISTS
	EN	All artists

PLUGIN_TRACKSTAT_SONGLIST_ALLARTISTS_FORGENRE_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_ALLARTISTS_FORGENRE
	EN	All artists in: 

PLUGIN_TRACKSTAT_SONGLIST_ALLARTISTS_FORYEAR_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_ALLARTISTS_FORYEAR
	EN	All artists from: 

PLUGIN_TRACKSTAT_SONGLIST_ALLARTISTS_FORPLAYLIST_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_ALLARTISTS_FORPLAYLIST
	EN	All artists in: 

PLUGIN_TRACKSTAT_SONGLIST_ALLGENRES
	EN	All genres

PLUGIN_TRACKSTAT_SONGLIST_ALLYEARS
	EN	All years

PLUGIN_TRACKSTAT_SONGLIST_ALLYEARS_FORARTIST_SHORT
	EN	Years

PLUGIN_TRACKSTAT_SONGLIST_ALLYEARS_FORARTIST
	EN	All years by: 

PLUGIN_TRACKSTAT_SONGLIST_ALLYEARS_FORGENRE_SHORT
	EN	Years

PLUGIN_TRACKSTAT_SONGLIST_ALLYEARS_FORGENRE
	EN	All years in: 

PLUGIN_TRACKSTAT_SONGLIST_ALLPLAYLISTS
	EN	All playlists
";
}
