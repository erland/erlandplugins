...
sub getNotCompletelyRatedHistoryAlbumsWeb {
	...
	if(defined($params->{'artist'})) {
	    ...
	    $sql = "select albums.id,min(case when track_statistics.rating is null then 0 else track_statistics.rating end) as minrating,avg(case when track_statistics.playCount is null then tracks.playCount else track_statistics.playCount end) as avgcount,max(track_statistics.lastPlayed) as lastplayed, max(track_statistics.added) from tracks,albums,contributor_track,track_statistics where tracks.url=track_statistics.url and tracks.album=albums.id and tracks.id=contributor_track.track and contributor_track.contributor=$artist and track_statistics.added$beforeAfter$beforeAfterTime group by tracks.album order by avgcount desc,minrating desc,$orderBy limit $listLength";
	    if($beforeAfter eq "<") {
...
sub getNotCompletelyRatedHistoryArtistsWeb {
	...
	if(defined($params->{'genre'})) {
		...
		my $year = $params->{'year'};
	    $sql = "select contributors.id,min(case when track_statistics.rating is null then 0 else track_statistics.rating end) as minrating,sum(case when track_statistics.playCount is null then tracks.playCount else track_statistics.playCount end) as sumcount,max(track_statistics.lastPlayed) as lastplayed, max(track_statistics.added) from tracks,contributor_track,contributors,track_statistics where  tracks.url=track_statistics.url and tracks.id=contributor_track.track and contributors.id = contributor_track.contributor and tracks.year=$year and track_statistics.added$beforeAfter$beforeAfterTime group by contributors.id having min(case when track_statistics.rating is null then 0 else track_statistics.rating end)=0 order by sumcount desc,minrating desc,$orderBy limit $listLength";
	    if($beforeAfter eq "<") {
		...
		my $playlist = $params->{'playlist'};
	    $sql = "select contributors.id,min(case when track_statistics.rating is null then 0 else track_statistics.rating end) as minrating,sum(case when track_statistics.playCount is null then tracks.playCount else track_statistics.playCount end) as sumcount,max(track_statistics.lastPlayed) as lastplayed, max(track_statistics.added) from tracks,contributor_track,contributors,playlist_track,track_statistics where  tracks.url=track_statistics.url and tracks.id=contributor_track.track and contributors.id = contributor_track.contributor and tracks.id=playlist_track.track and playlist_track.playlist=$playlist and track_statistics.added$beforeAfter$beforeAfterTime group by contributors.id having min(case when track_statistics.rating is null then 0 else track_statistics.rating end)=0 order by sumcount desc,minrating desc,$orderBy limit $listLength";
	    if($beforeAfter eq "<") {
	    ...
	    $sql = "select contributors.id,min(case when track_statistics.rating is null then 0 else track_statistics.rating end) as minrating,sum(case when track_statistics.playCount is null then tracks.playCount else track_statistics.playCount end) as sumcount,max(track_statistics.lastPlayed) as lastplayed, max(track_statistics.added) from tracks,contributor_track,contributors,track_statistics where  tracks.url=track_statistics.url and tracks.id=contributor_track.track and contributors.id = contributor_track.contributor and track_statistics.added$beforeAfter$beforeAfterTime group by contributors.id having min(case when track_statistics.rating is null then 0 else track_statistics.rating end)=0 order by sumcount desc,minrating desc,$orderBy limit $listLength";
	    if($beforeAfter eq "<") {
			$sql = "select contributors.id,min(case when track_statistics.rating is null then 0 else track_statistics.rating end) as minrating,sum(case when track_statistics.playCount is null then tracks.playCount else track_statistics.playCount end) as sumcount,max(track_statistics.lastPlayed) as lastplayed, max(track_statistics.added) as maxadded from tracks left join track_statistics on tracks.url = track_statistics.url join contributor_track on tracks.id=contributor_track.track join contributors on contributors.id = contributor_track.contributor group by contributors.id having (max(track_statistics.added) is null or max(track_statistics.added)<$beforeAfterTime) and min(case when track_statistics.rating is null then 0 else track_statistics.rating end)=0 order by sumcount desc,minrating desc,$orderBy limit $listLength";    
		}
	}
...
{
	return "
PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS
	EN	Not completely rated albums recently added

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORARTIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORARTIST
	EN	Not completely rated albums recently added by: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORGENRE_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORGENRE
	EN	Not completely rated albums recently added in: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORYEAR_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORYEAR
	EN	Not completely rated albums recently added from: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORPLAYLIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDALBUMS_FORPLAYLIST
	EN	Not completely rated albums recently added in: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDARTISTS
	EN	Not completely rated artists recently added

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDARTISTS_FORGENRE_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDARTISTS_FORGENRE
	EN	Not completely rated artists recently added in: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDARTISTS_FORYEAR_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDARTISTS_FORYEAR
	EN	Not completely rated artists recently added from: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDARTISTS_FORPLAYLIST_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDRECENTADDEDARTISTS_FORPLAYLIST
	EN	Not completely rated artists recently added in: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS
	EN	Not completely rated albums not recently added

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORARTIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORARTIST
	EN	Not completely rated albums not recently added by: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORGENRE_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORGENRE
	EN	Not completely rated albums not recently added in: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORYEAR_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORYEAR
	EN	Not completely rated albums not recently added from: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORPLAYLIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDALBUMS_FORPLAYLIST
	EN	Not completely rated albums not recently added in: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDARTISTS
	EN	Not completely rated artists not recently added

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDARTISTS_FORGENRE_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDARTISTS_FORGENRE
	EN	Not completely rated artists not recently added in: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDARTISTS_FORYEAR_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDARTISTS_FORYEAR
	EN	Not completely rated artists not recently added from: 

PLUGIN_TRACKSTAT_SONGLIST_NOTCOMPLETELYRATEDNOTRECENTADDEDARTISTS_FORPLAYLIST_SHORT
	EN	Artists
