...
sub getNotRatedTracksWeb {
	...
	if(defined($params->{'artist'})) {
	    ...
	    $sql = "select tracks.id,t1.playCount,t1.added,t1.lastPlayed,t1.rating from tracks join contributor_track on tracks.id=contributor_track.track and contributor_track.contributor=$artist and contributor_track.role in (1,4,5,6) left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 where tracks.audio=1 and t2.url is null group by tracks.id order by t1.playCount desc,$orderBy limit $listLength;";
	    $params->{'statisticparameters'} = "&artist=$artist";
	}elsif(defined($params->{'album'})) {
		my $album = $params->{'album'};
	    $sql = "select tracks.id,t1.playCount,t1.added,t1.lastPlayed,t1.rating from tracks left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 where tracks.audio=1 and tracks.album=$album and t2.url is null order by t1.playCount desc,$orderBy;";
	    $params->{'statisticparameters'} = "&album=$album";
	}elsif(defined($params->{'genre'})) {
		my $genre = $params->{'genre'};
	    $sql = "select tracks.id,t1.playCount,t1.added,t1.lastPlayed,t1.rating from tracks join genre_track on tracks.id=genre_track.track and genre_track.genre=$genre left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 where tracks.audio=1 and t2.url is null order by t1.playCount desc,$orderBy limit $listLength;";
	    $params->{'statisticparameters'} = "&genre=$genre";
	}elsif(defined($params->{'year'})) {
		my $year = $params->{'year'};
	    $sql = "select tracks.id,t1.playCount,t1.added,t1.lastPlayed,t1.rating from tracks left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 where tracks.audio=1 and t2.url is null and tracks.year=$year order by t1.playCount desc,$orderBy limit $listLength;";
	    $params->{'statisticparameters'} = "&year=$year";
	}elsif(defined($params->{'playlist'})) {
		my $playlist = $params->{'playlist'};
	    $sql = "select tracks.id,t1.playCount,t1.added,t1.lastPlayed,t1.rating from tracks join playlist_track on tracks.url=playlist_track.track and playlist_track.playlist=$playlist left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 where tracks.audio=1 and t2.url is null order by t1.playCount desc,$orderBy limit $listLength;";
	    $params->{'statisticparameters'} = "&playlist=$playlist";
	}else {
	    $sql = "select tracks.id,t1.playCount,t1.added,t1.lastPlayed,t1.rating from tracks left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 where tracks.audio=1 and t2.url is null order by t1.playCount desc,$orderBy limit $listLength;";
	}
...
sub getNotRatedTracks {
	...
	if($prefs->get("dynamicplaylist_norepeat")) {
		...
		$sql = "select tracks.id from tracks left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 left join dynamicplaylist_history on tracks.id=dynamicplaylist_history.id and dynamicplaylist_history.client='$clientid' where dynamicplaylist_history.id is null and tracks.audio=1 and t2.url is null order by t1.playCount desc,$orderBy limit $listLength;";
	}else {
		$sql = "select tracks.id from tracks left join track_statistics t1 on tracks.urlmd5=t1.urlmd5 left join track_statistics t2 on tracks.urlmd5=t2.urlmd5 and t2.rating>0 where tracks.audio=1 and t2.url is null order by t1.playCount desc,$orderBy limit $listLength;";
	}
...
sub getNotRatedArtistTracks {
	...
	if($prefs->get("dynamicplaylist_norepeat")) {
		my $clientid = $client->id;
		$sql = "select contributors.id,avg(case when track_statistics.rating is null then 0 else track_statistics.rating end) as avgrating,sum(ifnull(track_statistics.playCount,0)) as sumcount,max(track_statistics.lastPlayed) as lastplayed, max(track_statistics.added) as maxadded from tracks left join track_statistics on tracks.urlmd5 = track_statistics.urlmd5 join contributor_track on tracks.id=contributor_track.track and contributor_track.role in (1,4,5,6) join contributors on contributors.id = contributor_track.contributor left join dynamicplaylist_history on tracks.id=dynamicplaylist_history.id and dynamicplaylist_history.client='$client' where dynamicplaylist_history.id is null group by contributors.id having avgrating=0 order by sumcount desc,$orderBy limit $listLength";
	}else {
		$sql = "select contributors.id,avg(case when track_statistics.rating is null then 0 else track_statistics.rating end) as avgrating,sum(ifnull(track_statistics.playCount,0)) as sumcount,max(track_statistics.lastPlayed) as lastplayed, max(track_statistics.added) as maxadded from tracks left join track_statistics on tracks.urlmd5 = track_statistics.urlmd5 join contributor_track on tracks.id=contributor_track.track and contributor_track.role in (1,4,5,6) join contributors on contributors.id = contributor_track.contributor group by contributors.id having avgrating=0 order by sumcount desc,$orderBy limit $listLength";
	}
