...
sub getTopRatedHistoryTracksWeb {
	...
	if(defined($params->{'artist'})) {
	    ...
	    $sql = "select tracks.id,count(tracks.url) as sumcount,0 as added,max(track_history.played) as lastPlayed,avg(track_statistics.rating) as avgrating from tracks, track_history,playlist_track, track_statistics where tracks.url = track_history.url and tracks.url=track_statistics.url and tracks.id=playlist_track.track and playlist_track.playlist=$playlist and tracks.audio=1 and played$beforeAfter$beforeAfterTime group by tracks.url order by avgrating desc,sumcount desc,$orderBy limit $listLength;";
	    if($beforeAfter eq "<") {
...
sub getTopRatedHistoryTracks {
	...
	if(Slim::Utils::Prefs::get("plugin_trackstat_dynamicplaylist_norepeat")) {
		$sql = "select tracks.id,count(tracks.url) as sumcount,0 as added,max(track_history.played) as lastPlayed,avg(track_statistics.rating) as avgrating from tracks join track_history on tracks.url=track_history.url join track_statistics on tracks.url=track_statistics.url left join dynamicplaylist_history on tracks.id=dynamicplaylist_history.id where tracks.audio=1 and dynamicplaylist_history.id is null and played$beforeAfter$beforeAfterTime group by tracks.url order by avgrating desc,sumcount desc,$orderBy limit $listLength;";
		if($beforeAfter eq "<") {
		...
		$sql = "select tracks.id,count(tracks.url) as sumcount,0 as added,max(track_history.played) as lastPlayed,avg(track_statistics.rating) as avgrating from tracks, track_history, track_statistics where tracks.url = track_history.url and tracks.url=track_statistics.url and tracks.audio=1 and played$beforeAfter$beforeAfterTime group by tracks.url order by avgrating desc,sumcount desc,$orderBy limit $listLength;";
		if($beforeAfter eq "<") {
			$sql = "select tracks.id from tracks left join track_statistics on tracks.url = track_statistics.url where tracks.audio=1 and (track_statistics.lastPlayed is null or track_statistics.lastPlayed<$beforeAfterTime) order by track_statistics.rating desc,track_statistics.playCount desc,tracks.playCount desc,$orderBy limit $listLength;";
		}
	}
...
PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDRECENT_FORPLAYLIST_SHORT
	EN	Songs

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDRECENT_FORPLAYLIST
	EN	Top rated songs recently played in: 
...
PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDRECENTALBUMS_FORPLAYLIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDRECENTALBUMS_FORPLAYLIST
	EN	Top rated albums recently played in: 
...
PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDRECENTARTISTS_FORPLAYLIST_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDRECENTARTISTS_FORPLAYLIST
	EN	Top rated artists recently played in: 
...
PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDNOTRECENT_FORPLAYLIST_SHORT
	EN	Songs

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDNOTRECENT_FORPLAYLIST
	EN	Top rated songs not recently played in: 
...
PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDNOTRECENTALBUMS_FORPLAYLIST_SHORT
	EN	Albums

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDNOTRECENTALBUMS_FORPLAYLIST
	EN	Top rated albums not recently played in: 
...
PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDNOTRECENTARTISTS_FORPLAYLIST_SHORT
	EN	Artists

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDNOTRECENTARTISTS_FORPLAYLIST
	EN	Top rated artists not recently played in: 

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDRECENT_GROUP
	EN	Top rated

PLUGIN_TRACKSTAT_SONGLIST_TOPRATEDNOTRECENT_GROUP
	EN	Top rated
";
}
