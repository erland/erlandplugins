...
sub getTopRatedHistoryTracksWeb {
	...
	if(defined($params->{'artist'})) {
	    ...
	    $sql = "select tracks.id,count(tracks.url) as sumcount,0 as added,max(track_history.played) as lastPlayed,avg(track_statistics.rating) as avgrating from tracks, track_history,contributor_track,track_statistics where tracks.urlmd5 = track_history.urlmd5 and tracks.urlmd5=track_statistics.urlmd5 and tracks.id=contributor_track.track and contributor_track.role in (1,4,5,6) and contributor_track.contributor=$artist and tracks.audio=1 and played$beforeAfter$beforeAfterTime group by tracks.id order by avgrating desc,sumcount desc,$orderBy limit $listLength;";
	    if($beforeAfter eq "<") {
		...
		my $year = $params->{'year'};
	    $sql = "select tracks.id,count(tracks.url) as sumcount,0 as added,max(track_history.played) as lastPlayed,avg(track_statistics.rating) as avgrating from tracks, track_history, track_statistics where tracks.urlmd5 = track_history.urlmd5 and tracks.urlmd5=track_statistics.urlmd5 and tracks.year=$year and tracks.audio=1 and played$beforeAfter$beforeAfterTime group by tracks.id order by avgrating desc,sumcount desc,$orderBy limit $listLength;";
	    if($beforeAfter eq "<") {
...
sub getTopRatedHistoryTracks {
	...
	if($prefs->get("dynamicplaylist_norepeat")) {
		...
		$sql = "select tracks.id,count(tracks.url) as sumcount,0 as added,max(track_history.played) as lastPlayed,avg(track_statistics.rating) as avgrating from tracks join track_history on tracks.urlmd5=track_history.urlmd5 join track_statistics on tracks.urlmd5=track_statistics.urlmd5 left join dynamicplaylist_history on tracks.id=dynamicplaylist_history.id and dynamicplaylist_history.client='$clientid' where tracks.audio=1 and dynamicplaylist_history.id is null and played$beforeAfter$beforeAfterTime group by tracks.id order by avgrating desc,sumcount desc,$orderBy limit $listLength;";
		if($beforeAfter eq "<") {
