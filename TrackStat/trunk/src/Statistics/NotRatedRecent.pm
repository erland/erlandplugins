...
sub getNotRatedHistoryTracksWeb {
	...
	if(defined($params->{'artist'})) {
	    ...
	    $sql = "select tracks.url,count(track_history.url) as recentplayCount,0 as added,max(track_history.played) as lastPlayed,max(case when track_statistics.rating is null then 0 else track_statistics.rating end) as maxrating from tracks,track_history,track_statistics where tracks.url = track_history.url and tracks.url=track_statistics.url and tracks.audio=1 and tracks.album=$album and played$beforeAfter$beforeAfterTime and (track_statistics.rating is null or track_statistics.rating=0) group by track_history.url order by recentplayCount desc,maxrating desc,$orderBy;";
	    if($beforeAfter eq "<") {
	    ...
	    $sql = "select tracks.url,count(track_history.url) as recentplayCount,0 as added,max(track_history.played) as lastPlayed,max(case when track_statistics.rating is null then 0 else track_statistics.rating end) as maxrating from tracks,track_history,genre_track,track_statistics where tracks.url = track_history.url and tracks.url=track_statistics.url and tracks.id=genre_track.track and genre_track.genre=$genre and tracks.audio=1 and played$beforeAfter$beforeAfterTime and (track_statistics.rating is null or track_statistics.rating=0) group by track_history.url order by recentplayCount desc,maxrating desc,$orderBy limit $listLength;";
	    if($beforeAfter eq "<") {
	    ...
	    $sql = "select tracks.url,count(track_history.url) as recentplayCount,0 as added,max(track_history.played) as lastPlayed,max(case when track_statistics.rating is null then 0 else track_statistics.rating end) as maxrating from tracks,track_history,playlist_track,track_statistics where tracks.url = track_history.url and tracks.url=track_statistics.url and tracks.id=playlist_track.track and playlist_track.playlist=$playlist and tracks.audio=1 and played$beforeAfter$beforeAfterTime and (track_statistics.rating is null or track_statistics.rating=0) group by track_history.url order by recentplayCount desc,maxrating desc,$orderBy limit $listLength;";
	    if($beforeAfter eq "<") {
