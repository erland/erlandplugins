...
package Plugins::iPod::ConfigManager::TemplateContentParser;

use strict;
use base qw(Slim::Utils::Accessor);
use Plugins::iPod::ConfigManager::ContentParser;
our @ISA = qw(Plugins::iPod::ConfigManager::ContentParser);
	...
	if(defined($template->{lc($self->pluginId).'_plugin_template'})) {
		...
		my @pluginDirs = Slim::Utils::OSDetect::dirsFor('Plugins');
                for my $plugindir (@pluginDirs) {
                        if( -d catdir($plugindir,"iPod","Templates") && -e catfile($plugindir,"iPod","Templates",$templateFile)) {
                                if(defined($path)) {
                                        my $prevTimestamp = (stat ($path) )[9];
                                        my $thisTimestamp = (stat (catfile($plugindir,"iPod","Templates",$templateFile)) )[9];
                                        if($prevTimestamp<=$thisTimestamp) {
                                                $path = catfile($plugindir,"iPod","Templates",$templateFile);
                                        }
                                }else {
					$path = catfile($plugindir,"iPod","Templates",$templateFile);
				}
