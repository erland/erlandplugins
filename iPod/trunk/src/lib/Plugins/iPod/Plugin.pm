...
$prefs->migrate(1, sub {
	...
	$prefs->set('download_url',  Slim::Utils::Prefs::OldPrefs->get('plugin_ipod_download_url')   || 'http://erland.homeip.net/datacollection/services/DataCollection'  );
...
sub handleWebList {
	...
	if(!defined($params->{'donotrefresh'})) {
		if(defined($params->{'cleancache'}) && $params->{'cleancache'}) {
			my $cache = Slim::Utils::Cache->new("FileCache/iPod");
			$cache->clear();
		}
