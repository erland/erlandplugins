package GNUpod::CustomXMLhelper;
...
sub set_callbacks {
	my $cb = shift;
	$callbacks{'newfile'} =$cb->{'newfile'} if exists $cb->{'newfile'}; 
	$callbacks{'newpl'} =$cb->{'newpl'} if exists $cb->{'newpl'}; 
}
###############################################
...
sub eventer {
	...
	if($href->{Context}[1] eq "files") {
		...
		if(defined($callbacks{'newfile'})) {
			&{$callbacks{'newfile'}}($xh);            #call sub
		}else {
		...
		if(defined($callbacks{'newpl'})) {
			&{$callbacks{'newpl'}}(mkh($el, @it), $cpn, "pl"); #call sub
		}else {
		...
		if(defined($callbacks{'newpl'})) {
			&{$callbacks{'newpl'}}(mkh($el, @it), $cpn,"spl"); #Call sub  
		}else {
