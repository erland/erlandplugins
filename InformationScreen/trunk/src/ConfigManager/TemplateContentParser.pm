...
package Plugins::InformationScreen::ConfigManager::TemplateContentParser;

use strict;
use base qw(Slim::Utils::Accessor);
use Plugins::InformationScreen::ConfigManager::ContentParser;
our @ISA = qw(Plugins::InformationScreen::ConfigManager::ContentParser);
...
sub loadTemplate {
	...
	if(defined($template->{lc($self->pluginId).'_plugin_template'})) {
		...
		for my $plugindir (@pluginDirs) {
			if( -d catdir($plugindir,"InformationScreen","Templates") && -e catfile($plugindir,"InformationScreen","Templates",$templateFile)) {
				if(defined($path)) {
					my $prevTimestamp = (stat ($path) )[9];
					my $thisTimestamp = (stat (catfile($plugindir,"InformationScreen","Templates",$templateFile)) )[9];
					if($prevTimestamp<=$thisTimestamp) {
						$path = catfile($plugindir,"InformationScreen","Templates",$templateFile);
					}
				}else {
					$path = catfile($plugindir,"InformationScreen","Templates",$templateFile);
				}
