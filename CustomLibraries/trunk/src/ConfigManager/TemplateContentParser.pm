...
package Plugins::CustomLibraries::ConfigManager::TemplateContentParser;

use strict;
use base qw(Slim::Utils::Accessor);
use Plugins::CustomLibraries::ConfigManager::ContentParser;
our @ISA = qw(Plugins::CustomLibraries::ConfigManager::ContentParser);
...
sub loadTemplate {
	...
	if(defined($template->{lc($self->pluginId).'_plugin_template'})) {
		...
		for my $plugindir (@pluginDirs) {
			if( -d catdir($plugindir,"CustomLibraries","Templates") && -e catfile($plugindir,"CustomLibraries","Templates",$templateFile)) {
				if(defined($path)) {
					my $prevTimestamp = (stat ($path) )[9];
					my $thisTimestamp = (stat (catfile($plugindir,"CustomLibraries","Templates",$templateFile)) )[9];
					if($prevTimestamp<=$thisTimestamp) {
						$path = catfile($plugindir,"CustomLibraries","Templates",$templateFile);
					}
				}else {
					$path = catfile($plugindir,"CustomLibraries","Templates",$templateFile);
				}
