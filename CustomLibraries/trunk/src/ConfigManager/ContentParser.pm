...
package Plugins::CustomLibraries::ConfigManager::ContentParser;

use strict;
use base qw(Slim::Utils::Accessor);
use Plugins::CustomLibraries::ConfigManager::BaseParser;
our @ISA = qw(Plugins::CustomLibraries::ConfigManager::BaseParser);
...
sub checkContent {
	...
	if($globalcontext->{'source'} eq 'plugin' || $globalcontext->{'source'} eq 'builtin') {
		$xml->{'library'}->{'defaultitem'} = 1;
	}else {
		$xml->{'library'}->{'customitem'} = 1;
	}
	return 1;
}
