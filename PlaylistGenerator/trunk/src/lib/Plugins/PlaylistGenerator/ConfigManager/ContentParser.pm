...
sub checkContent {
	...
	if(defined($localcontext) && defined($localcontext->{'simple'})) {
		$xml->{'playlistdefinition'}->{'simple'} = 1;
	}
	if(defined($localcontext) && defined($localcontext->{'downloadidentifier'})) {
		$xml->{'playlistdefinition'}->{'downloadeditem'} = 1;
	}
	$xml->{'playlistdefinition'}->{'enabled'}=1;
	if($globalcontext->{'source'} eq 'plugin' || $globalcontext->{'source'} eq 'builtin') {
		$xml->{'playlistdefinition'}->{'defaultitem'} = 1;
	}else {
		$xml->{'playlistdefinition'}->{'customitem'} = 1;
	}
	return 1;
}
