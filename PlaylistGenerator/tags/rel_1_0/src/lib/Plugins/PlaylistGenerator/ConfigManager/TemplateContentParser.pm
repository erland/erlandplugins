...
sub loadTemplate {
	...
	if(defined($template->{lc($self->pluginId).'_plugin_template'})) {
		...
		for my $plugindir (@pluginDirs) {
			if( -d catdir($plugindir,"PlaylistGenerator","Templates") && -e catfile($plugindir,"PlaylistGenerator","Templates",$templateFile)) {
				if(defined($path)) {
					my $prevTimestamp = (stat ($path) )[9];
					my $thisTimestamp = (stat (catfile($plugindir,"PlaylistGenerator","Templates",$templateFile)) )[9];
					if($prevTimestamp<=$thisTimestamp) {
						$path = catfile($plugindir,"PlaylistGenerator","Templates",$templateFile);
					}
				}else {
					$path = catfile($plugindir,"PlaylistGenerator","Templates",$templateFile);
				}
