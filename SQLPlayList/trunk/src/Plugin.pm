	...
	if($params->{'text'}) {
		...
		if($playlist) {
			if(handleWebTestParameters($client,$params,$playlist)) {
				...
				$sql = replaceParametersInSQL($sql,getOffsetLimitParameters(0,10));
...
sub getTracksForPlaylist {
	...
	my $offsetLimitParameters = getOffsetLimitParameters($offset, $limit);
	$sqlstatements = replaceParametersInSQL($sqlstatements,$offsetLimitParameters);
...
sub getNextDynamicPlayListTracks {
	...
	my $result = getTracksForPlaylist($client,$playlist,$offset,$limit,$parameters);
